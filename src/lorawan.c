...
void showBuffer(const char *buff,uint8_t bufLen)
{
    printf( "showBuffer : %02X", buff[0] );
...
{
    printf("200 lorawan.c default_dev_eui\n");
    uint8_t boardId[8];
...
// BNN convertor for hex string to bytes
uint8_t hexToInt(uint8_t Char)
{
    if (Char>=0x30 & Char<=0x39) return Char-0x30;
    if (Char>=0x41 & Char<=0x46) return Char-0x41+10; // A is 10
    if (Char>=0x61 & Char<=0x66) return Char-0x61+10;
    return 0; 
}

uint8_t makeHexByte(uint8_t M,uint8_t L)
{
return (hexToInt(M)<<4) | hexToInt(L);
}
...
static void OnNetworkParametersChange( CommissioningParams_t* params )
{
    ...
    if (OtaaSettings != NULL) {
        ...
        printf("\n419 lorawan.c OnNetworkParametersChange()");
        printf("device_eui :");
       
        device_eui = OtaaSettings->device_eui;
        showBuffer(device_eui,8);
    ...
    if (device_eui != NULL) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            deviceEui[i]=makeHexByte(device_eui[i*2],device_eui[i*2+1]);
        }
 
        printf("475 lorawan.c after parsing\n");
        showBuffer(deviceEui,8);
        ...
        printf("480 lorawan.c after memcpy\n");
        showBuffer(params->DevEui,8);
    }
    ...
    if (app_eui != NULL) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            joinEui[i]=makeHexByte(app_eui[i*2],app_eui[i*2+1]);
        }
    ...
    if (app_key) {
        ...
        for (int i = 0; i < 16; i++) {
            ...
            appKey[i]=makeHexByte(app_key[i*2],app_key[i*2+1]);
        }
    ...
    if (app_session_key) {
        ...
        for (int i = 0; i < 16; i++) {
            ...
            appSessionKey[i]=makeHexByte(app_session_key[i*2],app_session_key[i*2+1]);
        }
    ...
    if (network_session_key) {
        ...
        for (int i = 0; i < 16; i++) {
            ...
            networkSessionKey[i]=makeHexByte(network_session_key[i*2],network_session_key[i*2+1]);
        }
    ...
    if (channel_mask != NULL) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            b[0]=makeHexByte(channel_mask[i*4],channel_mask[i*4+1]);
            b[1]=makeHexByte(channel_mask[i*4+2],channel_mask[i*4+3]);
